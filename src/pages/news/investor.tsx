...
export default function InvestorNewsPage() {
  ...
  useEffect(() => {
    if (isSignedIn && userMetadata?.userType && !['INVESTOR', 'VC_GROUP'].includes(userMetadata.userType)) {
  ...
  const {
    ...
    {},
    {
      enabled: isSignedIn && userMetadata?.userType && ['INVESTOR', 'VC_GROUP'].includes(userMetadata.userType),
        ...
        <main className="mx-auto max-w-7xl p-8">
          <div className="flex items-center justify-center h-64">
            <div className="rounded-3xl bg-card border border-white/10 p-8 text-center">
              <p className="text-gray-400">Access denied: This page is for investors only</p>
            </div>
          </div>
        </main>
      </div>
          ...
          <div className="rounded-3xl bg-card border border-white/10 p-12">
            <div className="flex items-center justify-center">
              <div className="flex items-center gap-3">
                ...
                <p className="text-gray-400 text-lg">Loading investor news...</p>
              </div>
            </div>
          </div>
        ) : error ? (
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-red-200/80 mb-4">
                  {error.message || 'Failed to load investor news. Please try again later.'}
                </p>
                <Button
