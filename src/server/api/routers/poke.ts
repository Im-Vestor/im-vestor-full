...
export const pokeRouter = createTRPCRouter({
  ...
  sendPokeToVcGroup: protectedProcedure
    .input(z.object({ vcGroupId: z.string(), message: z.string() }))
    .mutation(async ({ input, ctx }) => {
      const { vcGroupId, message } = input;

      // fetch vc group -> send poke in notification and email -> remove 1 poke from user

      const vcGroup = await db.vcGroup.findUnique({
        where: { id: vcGroupId },
        include: {
          user: true,
        },
