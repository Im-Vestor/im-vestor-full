...
export const NotionBlockRenderer: React.FC<NotionBlockRendererProps> = ({ blocks, className = '' }) => {
  const renderBlock = (block: BlockObjectResponse | PartialBlockObjectResponse): React.ReactNode => {
    ...
    const { type, id } = block;
    const value = (block as any)[type as keyof typeof block] as any;
    ...
    switch (type) {
      ...
      case 'embed':
        return (
          <div key={id} className="mb-6">
            <iframe
              src={value?.url}
              className="w-full h-96 rounded-lg border border-gray-300 dark:border-white"
        ...
        return (
          <div key={id} className="mb-4">
            <a
              ...
              className="block p-4 border border-gray-200 dark:border-white rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
            >
              <div className="flex items-start gap-3">
                <div className="flex-shrink-0 w-10 h-10 bg-gray-100 dark:bg-white rounded flex items-center justify-center">
                  🔗
                </div>
                <div className="flex-grow min-w-0">
                  <h4 className="font-medium text-gray-900 dark:text-white truncate">
                    {value?.caption?.[0]?.plain_text || value?.url || 'Bookmark'}
                  </h4>
      ...
      case 'file':
        ...
        const fileName = value?.name || value?.file?.name || 'Download file';
        return (
          <div key={id} className="mb-4">
            <a
              href={fileUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-white rounded-lg hover:bg-gray-200 dark:hover:bg-white transition-colors"
            >
          ...
          <div key={id} className="mb-6 overflow-x-auto">
            <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-lg">📊</span>
                <span className="font-medium text-yellow-800 dark:text-yellow-200">Table Content</span>
              </div>
              <p className="text-sm text-yellow-700 dark:text-yellow-300">
                Table content is best viewed directly in Notion
              </p>
            </div>
          </div>
        );
      ...
      default:
        return (
          <div key={id} className="mb-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 border-l-4 border-yellow-400 text-yellow-800 dark:text-yellow-200">
            <p className="text-sm">
              <span className="font-medium">Unsupported block type:</span> <code className="font-mono bg-yellow-100 dark:bg-yellow-800 px-1 py-0.5 rounded">{type}</code>
            </p>
          </div>
        );
    }
...
const RichText: React.FC<{ text: any[] }> = ({ text }) => {
  ...
  return (
    <>
      {text.map((value, index) => {
        ...
        if (code) className += 'font-mono bg-gray-200 dark:bg-white px-1 py-0.5 rounded text-sm ';
